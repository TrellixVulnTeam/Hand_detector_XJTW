...
class PacketPresenceCalculator : public CalculatorBase {
 public:
  static ::mediapipe::Status GetContract(CalculatorContract* cc) {
    cc->Inputs().Tag("PACKET0").SetAny();
    cc->Outputs().Tag("PRESENCE0").Set<bool>();

    cc->Inputs().Tag("PACKET1").SetAny();
    cc->Outputs().Tag("PRESENCE1").Set<bool>();
  ...
  ::mediapipe::Status Process(CalculatorContext* cc) final {
    cc->Outputs()
        .Tag("PRESENCE0")
        .AddPacket(MakePacket<bool>(!cc->Inputs().Tag("PACKET0").IsEmpty())
                       .At(cc->InputTimestamp()));

    cc->Outputs()
        .Tag("PRESENCE1")
        .AddPacket(MakePacket<bool>(!cc->Inputs().Tag("PACKET1").IsEmpty())
                       .At(cc->InputTimestamp()));
