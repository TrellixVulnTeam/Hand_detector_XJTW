...
::mediapipe::Status RunMPPGraph() {
  ...
  while (grab_frames) {
    ...
    std::unique_ptr<std::vector<::mediapipe::NormalizedLandmarkList>> output_landmarks;
    ...
    std::cout << std::endl << "New list of landmarks:" << std::endl << std::flush;
    for (const ::mediapipe::NormalizedLandmarkList& landmark : output_landmarks) 
    {
      double x = landmark.landmark(0).x();
      double y = landmark.landmark(0).y();
      double z = landmark.landmark(0).z();
